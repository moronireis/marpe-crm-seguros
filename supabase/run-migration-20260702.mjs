...
const STATEMENTS = [
  {
    name: 'DROP old marpe_deal_activities_type_check constraint',
    sql: `ALTER TABLE public.marpe_deal_activities DROP CONSTRAINT IF EXISTS marpe_deal_activities_type_check;`,
  },
  {
    name: 'VERIFY single CHECK constraint remains',
    sql: `SELECT conname FROM pg_constraint WHERE conrelid = 'public.marpe_deal_activities'::regclass AND contype = 'c';`,
    verify: true,
  },
];
...
async function run() {
  console.log('=== Marpe CRM Migration 2026-07-02: Fix Duplicate Activity Constraint ===\n');
